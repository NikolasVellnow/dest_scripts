...
bcftools view -Ou -S samples_Linvilla.txt first_5000.bcf.gz -O b -o linvilla_first_5000.bcf.gz


# create index of bcf-file
bcftools index linvilla_first_5000.bcf.gz

# save header for easier understanding of file
bcftools view -h linvilla_first_5000.bcf.gz > header_linvilla_first_5000.txt

# exclude indels (although I think there where already no indels anymore)
bcftools view -Ou -I linvilla_first_5000.bcf.gz -O b -o linvilla_first_5000.bcf.gz

# exclude SNPs from "messy" chromosomes and only biallelic SNPs with at least one sample having total depth of "."
bcftools view -Ou -e'CHROM="X" | CHROM="Y" | CHROM="4" | FORMAT/DP="."' -m2 -M2 -v snps linvilla_first_5000.bcf.gz -O b -o linvilla_first_5000_filtered.bcf.gz

bcftools plugin counts linvilla_first_5000_filtered.bcf.gz

# get chromosome, position, alternative counts (AD), reference counts (RD) and total depths (DP) for all samples
bcftools query -f'%CHROM\t%POS\t[%AD\t]\t[%RD\t]\t[%DP\t]\n' linvilla_first_5000_filtered.bcf.gz | head -10
